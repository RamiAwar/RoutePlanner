...
public:
    ...
    // Important note on returning by reference:
    // If returned a copy only, we can't modify the original m_Nodes vector inside the object
    // We'd only be acting on a copy of that object or assigning to a stack location
    // Returning by reference allows modification of the type m_Nodes[2] = x; modifying the object member variable itself
    auto &Nodes() const noexcept { return m_Nodes; } // return nodes address ( reference return type )
